...
class SubCategoryChooser(FieldPanel):
    """
    FieldPanel with pop-over chooser style form to select subcategories from a ClusterableModel/Orderable class pair.
    Chooser modal will present orderable instances (subcategories) grouped by parent ClusterableModel instace (category).
    ...
    Inputs:
      ...
      category_sort_order (optional) - sort order for the category groups on the chooser, defaults to 'sort_order' meta value
    ...
    def __init__(
            ...
            add_button_text=_("Select Category"), 
    ...
    def on_model_bound(self):
        ...
        if settings.DEBUG: 
            if not issubclass(self.category, ClusterableModel):
                raise ImproperlyConfigured("Category model must inherit from ClusterableModel.")
            if not issubclass(self.subcategory, Orderable):
                raise ImproperlyConfigured("Subcategory model must inherit from Orderable.")
            related_field = getattr(self.category, self.subcategory_related_name, False);
    ...
    class BoundPanel(FieldPanel.BoundPanel):
        ...
        def get_categories(self):
            ...
            categories = self.panel.category.objects.prefetch_related(
                Prefetch(self.panel.subcategory_related_name, queryset=self.panel.subcategory.objects.order_by(self.panel.category_sort_order))
            )
        ...
        def get_context_data(self, parent_context=None):
            ...
            if initial_value:
                parental_key_field = getattr(self.panel.category, self.panel.subcategory_related_name).field.name
                initial_value = f'{str(getattr(initial_value, parental_key_field))} - {str(initial_value)}'
